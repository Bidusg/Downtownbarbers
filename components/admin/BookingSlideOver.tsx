...
const STATUS_STYLE: Record<string, string> = {
  confirmed: 'text-forest-light bg-forest/15',
      ...
      {booking && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-full w-full sm:w-[400px] bg-ink-soft border-l border-stroke-dark z-50 flex flex-col overflow-hidden shadow-2xl"
          >
            {/* Color indicator strip */}
            <div className="h-[3px] shrink-0" style={{ backgroundColor: color }} />

            {/* Header */}
            <div className="flex items-start justify-between px-5 py-4 border-b border-stroke-dark shrink-0">
              <div>
                {mode === 'edit' ? (
                  <button onClick={() => setMode('view')} className="flex items-center gap-2 text-muted hover:text-cream text-xs font-sans uppercase tracking-[0.16em] transition-colors mb-1">
                ...
                <p className="text-cream font-sans font-semibold text-base">{booking.customer_name}</p>
                <p className="text-muted text-xs font-sans mt-0.5">
                  {formatDate(booking.booking_date)} kl {booking.booking_time.slice(0, 5)}
                </p>
              </div>
              <button onClick={onClose} className="text-muted hover:text-cream transition-colors mt-0.5">
                ...
                <div className="p-5">
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    <div className="flex gap-3">
                      ...
                      <span className={`text-[10px] font-sans uppercase tracking-[0.18em] px-2 py-0.5 ${STATUS_STYLE[booking.status] ?? 'text-muted bg-ink-mid'}`}>
                  ...
                  <div className="space-y-2 mb-4">
                    {booking.status !== 'confirmed' && (
                      <button
                        ...
                        disabled={submitting}
                        className="w-full py-2.5 bg-forest hover:bg-forest-mid text-cream text-xs font-sans uppercase tracking-[0.16em] transition-colors disabled:opacity-50"
                      >
                        Bekreft
                      </button>
                    )}
                    ...
                    <button
                      ...
                      className="w-full py-2.5 border border-stroke-dark hover:border-forest/50 text-cream/70 hover:text-cream text-xs font-sans uppercase tracking-[0.16em] transition-colors"
                    >
                      Rediger
                    </button>
                    <a
                      href={`tel:${booking.customer_phone}`}
                      className="flex items-center justify-center gap-2 w-full py-2.5 border border-stroke-dark hover:border-forest/40 text-muted hover:text-cream text-xs font-sans uppercase tracking-[0.16em] transition-colors"
                    >
                  ...
                  {confirmCancel && (
                    <div className="border border-red-500/30 bg-red-900/10 p-4">
                      <p className="text-red-300 text-sm font-sans mb-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="flex-1 py-2 border border-stroke-dark text-muted hover:text-cream text-xs font-sans uppercase tracking-[0.14em] transition-colors"
                        >
                          Nei
                        </button>
                      </div>
