...
export default async function OmsetningDrill({
  searchParams,
}: {
  ...
  // Standard: inneværende måned hvis ingen parameter.
  const now = new Date();
  const dag = sp.dag?.match(/^\d{4}-\d{2}-\d{2}$/) ? sp.dag : undefined;
  let mnd = sp.mnd?.match(/^\d{4}-\d{2}$/) ? sp.mnd : undefined;
  if (!dag && !mnd) {
    mnd = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;
  }

  const back = (
    <Link href="/admin/regnskap" className="text-sm text-muted hover:text-fg">
  ...
  if (dag) {
    ...
    const title = new Date(dag + "T12:00:00Z").toLocaleDateString("nb-NO", {
      weekday: "long",
      day: "numeric",
      month: "long",
      year: "numeric",
    ...
    return (
      <div className="mx-auto max-w-6xl space-y-8">
        <div className="space-y-1">
          ...
          <Link
            href={`/admin/omsetning?mnd=${dag.slice(0, 7)}`}
  ...
  /* ---------- MÅNEDSVISNING ---------- */
  const [my, mm] = (mnd as string).split("-").map(Number);
  ...
  const [{ total, rows, byBarber, byMethod }, days] = await Promise.all([
    ...
    getDaysInMonth(mnd as string),
  ]);
  const maxDay = Math.max(1, ...days.map((d) => d.nok));
  const avg = rows.length ? Math.round(total / rows.length) : 0;
          ...
          <ul>
            {days.map((d) => (
              <li key={d.key} className="border-b border-line last:border-0">
                <Link
                  href={`/admin/omsetning?dag=${d.key}`}
                  className="flex items-center gap-4 px-6 py-3 transition-colors hover:bg-surface-2"
                >
                  <span className="w-14 text-sm text-fg-soft tabular-nums">{d.label}</span>
                  <span className="h-2 flex-1 overflow-hidden rounded-full bg-line">
                    <span
                      className="block h-full rounded-full bg-accent-soft"
                      style={{ width: `${Math.round((d.nok / maxDay) * 100)}%` }}
                    />
                  </span>
                  <span className="w-28 text-right text-sm font-medium tabular-nums">{nok(d.nok)}</span>
                </Link>
              </li>
            ))}
          </ul>
