...
import { getShopToday } from "@/lib/dashboard-queries";

export default async function KasseDashboard() {
  await requireRole(["shop", "admin"]);
  const today = await getShopToday();
    ...
    <div className="min-h-screen bg-canvas text-fg">
      ...
      <main className="mx-auto max-w-3xl space-y-8 p-6">
        ...
        <section className="border border-line bg-surface p-6">
          ...
          {today.nextUp.length === 0 ? (
            <p className="py-4 text-sm text-muted">Ingen flere timer i dag.</p>
          ) : (
